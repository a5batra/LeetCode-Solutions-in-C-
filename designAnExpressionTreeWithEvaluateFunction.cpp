//
// Created by Ankit Batra on 4/21/21.
//
 ...
 * appear before their operators. For example, the postfix tokens of the expression 4*(5-(7+2)) are represented
 ...
 * For example, is your design able to support additional operators without making changes to your existing
...
public:
    Node* buildTree(vector<string>& postfix) {
        ...
        for (string s : postfix) {
            if (isdigit(s[0])) {
                myStack.push(new NumNode(stoi(s)));
            }
            else {
                ...
                myStack.pop();
                myStack.push(new OpNode(s[0], left, right));
            }
        }
        return myStack.top();
    }
