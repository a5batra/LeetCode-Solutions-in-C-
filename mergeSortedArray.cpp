//
// Created by Ankit Batra on 10/1/20.
//

/* Given two sorted integer arrays nums1 and nums2, merge nums2 into nums1 as one sorted array.
 * Note:
 * The number of elements initialized in nums1 and nums2 are m and n respectively.
 * You may assume that nums1 has enough space (size that is equal to m + n) to hold additional elements from nums2. */

#include <iostream>
#include <sstream>
#include <vector>

using std::string;
using std::vector;

class Solution {
public:
    static void merge(vector<int>& nums1, int m, vector<int>& nums2, int n) {
        int i = m - 1, j = n - 1, k = nums1.size() - 1;

        while (i >= 0 && j >= 0) {
//            std::cout << "Nums1: " << nums1[i] << " " << "Nums2: " << nums2[j] << std::endl;
            if (nums1[i] >= nums2[j]) {
                nums1[k--] = nums1[i--];
            }
            else {
                nums1[k--] = nums2[j--];
            }
        }

        while (j >= 0)
            nums1[k--] = nums2[j--];
    }
};

int main() {
    vector<int> nums1;
    vector<int> nums2;
    int number, m, n;
    string line1, line2;

    std::cout << "Enter elements of nums1 separated by space: " << std::endl;
    std::getline(std::cin, line1);

    std::istringstream stream1(line1);
    while (stream1 >> number)
        nums1.push_back(number);

    std::cout << "Enter elements of nums2 separated by space: " << std::endl;
    std::getline(std::cin, line2);

    std::istringstream stream2(line2);
    while (stream2 >> number)
        nums2.push_back(number);

    std::cout << "Enter size of first array: " << std::endl;
    std::cin >> m;

    std::cout << "Enter size of second array: " << std::endl;
    std::cin >> n;

    Solution::merge(nums1, m, nums2, n);

    for (int i = 0; i < nums1.size(); ++i)
        std::cout << nums1[i] << " ";

    return 0;
}